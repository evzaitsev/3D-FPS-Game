...
void Octree::Update(CXMMATRIX W)
{
	Update(W, mRoot);
}

void Octree::Update(CXMMATRIX World, OctreeNode* parent)
{
	...
	if (XNA::IntersectAxisAlignedBoxFrustum(&parent->Bounds, &localspaceFrustum) != 0)
	{
		parent->Visible = true;
	...
	/*	for (int i = 0; i < 8; ++i)
		{
			INT intersect = XNA::IntersectAxisAlignedBoxFrustum(&parent->Children[i]->Bounds, &localspaceFrustum);
		    
			if (intersect == 2)
				SetVisible(parent->Children[i], true);

			else if (intersect == 1)
			{
				parent->Children[i]->Visible = true;
				Update(World, parent->Children[i]);
			}
			else
			{
				parent->Children[i]->Visible = false;

				SetVisible(parent->Children[i], false);
			}
...
void Octree::SetVisible(OctreeNode* parent, bool visible)
{
	if (!parent->IsLeaf)
	{
		for (int i = 0; i < 8; ++i)
		{
			parent->Children[i]->Visible = visible;

			SetVisible(parent->Children[i], visible);
		}
	}
	else
...
void Octree::AddObject(Object& inout)
{
	OctreeNode** ppNode = AddObject(mRoot, &inout.Box);

	inout.node = ppNode;
}

//it is responsibility of the user to make sure that boxes
//are in same space 

//yet in its development stage
OctreeNode** Octree::AddObject(OctreeNode* parent, XNA::AxisAlignedBox* box)
{
	//returns lowest node which intersected object
	static int i = 0;

	if (XNA::IntersectAxisAlignedBoxAxisAlignedBox(&parent->Bounds, box) != 0)
	{
		for (int i = 0; i < 8; ++i)
		{
			if (!parent->IsLeaf)
			{
				if (XNA::IntersectAxisAlignedBoxAxisAlignedBox(&parent->Children[i]->Bounds, box) != 0)
				{
					return AddObject(parent, box);
				}

			}else{

				if (i > 1)
					ShowError("Inbetween nodes");
